#!/usr/bin/env python3
"""
tclscanner.py is a wrapper script for tclscan.
...
github: https://github.com/simonkowallik/tclscanner
license: MIT
"""
import argparse
import json
...
def main():
    """
    main function reads directory argument and runs process_directory.
    prints results as json to stdout
    """

    parser = argparse.ArgumentParser(
        prog="tclscanner",
        description="`tclscanner` is a wrapper for `tclscan`.",
        epilog="LICENSE: MIT, homepage: https://github.com/simonkowallik/tclscanner",
    )
    parser.add_argument("--file", help="analyze provided file", nargs="?", type=argparse.FileType("r"), default=None)
    parser.add_argument(
        "-d",
        "--directory",
    ...
    if args.file is not None:
        ...
        try:
            ...
            results : dict = {}
            results[str(args.file.name)] = run_scan(filecontent)
        except UnicodeDecodeError as error:
                print(f"ERROR: could not read file, error:{error}")
    else:
        # process directory
